...
if [ -n "$EXISTING_PID" ]; then
  ...
  anvil \
    --load-state "$ROOT_DIR/anvil-state.json" \
...
cast rpc anvil_mine 20 --rpc-url "$RPC_URL"
...
if [ -z "$PRIVATE_KEY" ]; then
  echo "❌ 缺少 PRIVATE_KEY"
  echo "请在 frontend/.env.local 中添加："
  echo "PRIVATE_KEY=0x..."
  exit 1
fi
...
# 第五步：检查 deployment 文件
echo "📝 检查 deployment 文件..."

if [ ! -f "$DEPLOY_FILE" ]; then
  echo "❌ deployment 文件不存在:"
  echo "$DEPLOY_FILE"
  exit 1
fi

# 第六步：读取部署地址
GAME_ADDRESS="$(jq -r '.GAME_ADDRESS' "$DEPLOY_FILE")"
ROD_ADDRESS="$(jq -r '.ROD_ADDRESS' "$DEPLOY_FILE")"
VRF_ADDRESS="$(jq -r '.VRF_ADDRESS' "$DEPLOY_FILE")"
