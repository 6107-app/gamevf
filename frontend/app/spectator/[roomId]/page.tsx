...
const STATUS_LABEL: Record<PlayerStatus, string> = {
  waiting: "Waiting", fishing: "Fishing", caught: "Hooked", locked: "Locked",
};
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: "11px", color: "var(--brown-light)", fontWeight: 600 }}>
            ...
            {player.recastCount > 0 && ` · ${player.recastCount} recast${player.recastCount > 1 ? "s" : ""}`}
          </div>
        </div>
...
function EventItem({ event }: { event: LiveEvent }) {
  const typeConfig: Record<LiveEvent["type"], { label: string; color: string; icon: string }> = {
    ...
    locked: { label: "Locked", color: "var(--mint-dark)", icon: "✅" },
...
export default function SpectatorPage() {
  ...
  useEffect(() => {
    ...
    const onDiceRolled = (
      ...
      setEvents(prev => [{
        ...
        playerEns: ens, playerAvatar: avatar,
        type: "dice" as const,
        diceResult: mod >= 0 ? `🎲 Bonus +${mod}%` : `🎲 Penalty ${mod}%`,
      }, ...prev].slice(0, 30));
    };
    ...
    if (marketReady) {
      ...
      const onBetPlaced = (
        ...
        setEvents(prev => [{
          ...
          type: "bet" as const,
          diceResult: `Backing ${winner?.ens ?? shortenAddr(predictedWinner)} · ${(Number(amount) / 1e18).toFixed(4)} ETH`,
        }, ...prev].slice(0, 30));
  ...
  const handlePlaceBet = useCallback(async () => {
    ...
    if (elapsed >= 180)            { setBetError("The game has ended — betting is closed"); return; }
          ...
          {hasBet ? (
            ...
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              ...
              <div style={{ fontSize: "13px", color: "var(--brown-light)", marginBottom: "16px" }}>
                Backing {players[userBetIdx!].ens}
              </div>
              <div style={{
              ...
              <div style={{ marginTop: "14px", fontSize: "11px", color: "var(--brown-light)" }}>
                Collect your winnings after the game ends
              </div>
            </div>
          ) : (
            /* ── Betting form ── */
            <>
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: "10px", color: "var(--brown-light)" }}>
                          {fmtScore(p.score)} pts · {getPlayerOdds(p.addr).toFixed(1)}% bet share
                        </div>
                      </div>
                      {selectedIdx === p.idx && (
              ...
              >
                {txPending ? "⏳ Confirming…" : "🎯 Confirm Bet"}
              </button>
              ...
              <div style={{
                ...
                5% platform fee · Winners share the pool proportionally by weighted bet
              </div>
            </>
          )}
