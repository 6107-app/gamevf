...
export default function RodDetailPage() {
  ...
  const handleRepair = async () => {
    ...
    try {
      ...
      if (SIMULATE) {
        ...
        alert(`Simulated repair: cost ${sim.price && sim.price.toString && sim.price.toString() !== '0' ? ethers.formatEther(sim.price) + ' ETH' : getFullRepairCostLocal(rod.type as any, rod.level) + ' tokens'}, estimated gas ${sim.gasEstimate ? sim.gasEstimate.toString() : 'n/a'}`);
      ...
      await tx.wait();
      alert('✅ Rod repaired successfully!');
    } catch (e: any) {
      console.error(e);
      const code = e?.code || e?.error?.code;
      if (code === 4001) alert('Transaction rejected by user');
  ...
  const handlePartialRepair = async (restore: number) => {
    ...
    try {
      ...
      if (SIMULATE) {
        alert(`Simulated partial repair: restore +${restore} durability, cost ${quoted} ETH`);
        setIsProcessing(false);
        return;
      }
      if (!(window as any).ethereum) return alert('Please install a wallet');
      ...
      alert('Transaction submitted, waiting for confirmation');
      await tx.wait();
      alert(restore === 100 ? '✅ Rod fully repaired!' : '✅ Rod partially repaired!');
    } catch (e: any) {
      console.error(e);
      const code = e?.code || e?.error?.code;
      if (code === 4001) alert('Transaction rejected by user');
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (SIMULATE) {
        ...
        alert(`Simulated upgrade: cost ${simPriceStr}, estimated gas ${sim.gasEstimate ? sim.gasEstimate.toString() : 'n/a'}, possible attribute: ${simulated.attribute} +${simulated.incrementPercent}%`);
        setIsProcessing(false);
      ...
      alert('Transaction submitted, waiting for confirmation');
      await tx.wait();
      alert('🔔 Upgrade transaction complete. The result will be determined by the on-chain VRF event.');
    } catch (e: any) {
      console.error(e);
      const code = e?.code || e?.error?.code;
      if (code === 4001) alert('Transaction rejected by user');
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              >
                {isProcessing
                  ? "Upgrading..."
                  : canUpgrade
                    ? `⬆️ Upgrade to +${rod.level + 1} (${upgradeFee ? upgradeFee + ' ETH' : 'Loading...'})`
                    : "✓ Max Level Reached"}
              </button>
            </div>
          </div>
            ...
            <div>
              ...
              <div style={{
                ...
                {[
                  ...
                  { label: "Luck", value: `${rod.attributes.luckBonus ?? Math.round(rod.attributes.luckBps / 100)}%`, hint: "Increases rare catch probability" },
                  { label: "Stability", value: `${Math.round((rod.attributes.stabilityBps ?? 0) / 100)}%`, hint: "Reduces missed catch / Debuff impact" },
                ].map(attr => (
            ...
            <div>
              ...
              <div style={{ display: "grid", gap: "8px" }}>
                <div style={{ fontSize: "13px", color: "var(--brown)", lineHeight: "1.7" }}>
                  Rods can be upgraded up to +5. A successful upgrade randomly improves one of Speed / Weight / Luck / Stability.
                </div>
                <div style={{ fontSize: "13px", color: "var(--brown)", lineHeight: "1.7" }}>
                  Success rates by level: 100% / 85% / 65% / 45% / 25%. Failure does not reduce level, but the fee is non-refundable.
                </div>
                <div style={{ fontSize: "13px", color: "var(--brown)", lineHeight: "1.7" }}>
                  Stat increments vary by level; +0 through +5 each use different random ranges.
                </div>
              </div>
            </div>
        ...
        >
          ...
          >
            ...
            <p style={{
              ...
              Upgrade <strong>{rodType.name}</strong> from <strong>+{rod.level}</strong> to <strong>+{rod.level + 1}</strong>. One of the four attributes will be randomly improved.
            </p>
            ...
            >
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{ color: "var(--brown-light)" }}>Upgrade Cost</span>
            ...
            >
              ⚠️ A failed upgrade will not reduce your rod's level, but the upgrade fee is non-refundable.
            </div>
